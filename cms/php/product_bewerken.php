...
$sqlProduct = $mysqli -> prepare("SELECT id, naam, model, merk, prijs, prijs_korting, kleur, frameMaat, extras, paginaurl, status FROM digifixx_producten WHERE id = ?") or die ($mysqli->error.__LINE__);
$productId = $_GET['id'];
$sqlProduct->bind_param('i',$productId);
$sqlProduct->execute();
$sqlProduct->store_result();
$sqlProduct->bind_result($idProduct, $titelProduct, $modelProduct, $merkProduct, $prijsProduct, $prijsKProduct, $kleurProduct, $frameMaatProduct, $extraProduct, $urlProduct, $statusProduct);
$sqlProduct->fetch();
